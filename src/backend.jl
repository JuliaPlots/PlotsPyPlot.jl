...
py_color(cs::AbstractVector) = map(py_color, cs)
...
# py_marker(markers::AbstractVector) = map(py_marker, markers)
function py_marker(markers::AbstractVector)
...
function get_locator_and_formatter(vals::AbstractVector)
    pyticker["FixedLocator"](1:length(vals)), pyticker["FixedFormatter"](vals)
end

function add_pyfixedformatter(cbar, vals::AbstractVector)
...
function py_bbox(v::AbstractVector)
...
function py_add_series(plt::Plot{PyPlotBackend}, series::Series)
    ...
    if series[:markershape] != :none && st in (:path, :scatter, :path3d,
                                          ...
                                          :bar)
        markercolor = if any(typeof(series[arg]) <: AbstractVector for arg in (:markercolor, :markeralpha)) || series[:marker_z] != nothing
    ...
    if fillrange != nothing && st != :contour
        for (i, rng) in enumerate(iter_segments(series))
            ...
            n = length(dim1)
            args = if typeof(fillrange) <: Union{Real, AbstractVector}
